...
func (api NeutrinoAPI) BadWordFilter(params url.Values) (map[string]interface{}, error) {
	url, err := url.Parse("https://neutrinoapi.net/bad-word-filter")
	if err != nil {
...
func main() {
	...
	params.Add("censor-character", "λ") // The character to use to censor out the bad words found
	params.Add("catalog", "strict") // Which catalog of bad words to use
	params.Add("content", "[Ф][Ö][©][ش𐹧][的] Some HTML with UTF chars...") // The content to scan
